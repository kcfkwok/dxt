...
def make_dxt_kz_A4L(dt,latv,longv,place,timezone):
    second=0
    if latv <0:
    ...
    paper.draw_outline()
    #paper.draw_MAX_RECT()
    OFS_X=int(10 * MM_UNIT + paper.w_px/2.0)
    ...
    xc = config.xc1 + int(paper.w_px/2.0)
    yc = config.yc1
    ...
    layer_bg.im.paste(im_bg, (int(paper.w_px/2.0),0))
    ...
    x= int(3*MM_UNIT + paper.w_px/2.0)
    y= int(150*MM_UNIT)
...
if __name__=='__main__':
    ...
    fnx='dxt_kz_%s_%02d_%02dt%02d_%02d_%s_%s_%s_A4L.pdf' % (year,month,day,
                                                           hour,minute,
                                                          latv,longv,place)
    fn = '/kcf/dxtc2/%s' % fnx
    ...
    paper= make_dxt_kz_A4L(dt,latv,longv,place,timezone)
